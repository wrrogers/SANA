...
# Need to fix this to ignore non .dcm files (like above)
def load_scans(path):
	ds = []
	for p in os.listdir(path):
	    ds.append(dicom.read_file(os.path.join(path, p), force=True))

	image3d = np.array([s.pixel_array for s in ds])
...
def maskit(images, ctrs2d):
    ...
    for img, c in zip(images, ctrs2d):
        dst = np.zeros(shape = (512, 512), dtype = 'int16') # An empty mask for CV2 to compute the normalization
        normalized = cv2.normalize(img,dst,0,255,cv2.NORM_MINMAX)
...
# All put together
def process_all(path, max):
    cropped, maxes = mask_crop(path, max)
    padded = pad(cropped, maxes, max)
    return padded

#data = process_all('C:/Users/wrrog/Desktop/Data', max = 10)
